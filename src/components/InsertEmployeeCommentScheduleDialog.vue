  ...
  >
    <v-card class="rounded-lg">
      <v-card-title class="d-flex align-center px-6 py-2 bg-teal-darken-1">
        ...
        <span class="card-title text-white">插入排程</span>
        <v-spacer />
        <v-btn
          icon
          variant="plain"
...
const insertSchedule = async () => {
  ...
  try {
    ...
    const { data } = await apiAuth.post('/employee-comments/schedules/insert', {
    ...
    createSnackbar({
      text: error?.response?.data?.message || '插入排程失敗',
      snackbarProps: { color: 'red-lighten-1' }
    })
  } finally {

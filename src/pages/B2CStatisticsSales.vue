...
const isCurrentMonthActive = ref(false) // 添加狀態追蹤是否正在顯示本月資料
...
const performSearch = async () => {
  ...
  try {
    ...
    // 使用公共API端點
    const endpoint = '/customerInquiries/public/all'
...
const resetSearch = () => {
  ...
  quickSearch.value = ''
  isCurrentMonthActive.value = false // 重置本月狀態
  performSearch()
}
...
const addProgressNote = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (simpleDialog.value.requireNote && simpleDialog.value.pendingResult) {
        // 更新詢問結果
        const endpoint = `/customerInquiries/public/${simpleDialog.value.id}`

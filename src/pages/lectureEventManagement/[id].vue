<template>
  <v-container max-width="2100">
      ...
      <template v-else-if="lectureEventData.lectureName">
        ...
        >
          <div class="mb-2 d-flex align-center">
            <span class="lecture-title ">{{ lectureEventData.lectureName }}</span>
            <v-btn
              icon
              variant="text"
              color="blue-grey-darken-2"
                  ...
                  <tr :class="{ 'odd-row': index % 2 === 0, 'even-row': index % 2 !== 0 }">
                    ...
                    <td class="text-center">
                      ...
                      <v-btn
                        ...
                        size="small"
                        color="red-darken-1"
                        @click="openDeleteParticipantDialog(item)"
                      >
    ...
    <ConfirmDeleteDialog
      v-model="deleteParticipantDialog.open"
      title="確認刪除參加者"
      message="確定要刪除此參加者嗎？此操作無法恢復。"
...
const deleteParticipantDialog = reactive({
  open: false,
  item: null
})
...
const currentHistoryParticipantId = ref(null)
...
const openDeleteParticipantDialog = (item) => {
  deleteParticipantDialog.item = item
  deleteParticipantDialog.open = true
}

const confirmDeleteParticipant = async () => {
  const item = deleteParticipantDialog.item
  deleteParticipantDialog.item = null
  if (!item?._id) return

  try {
    const { data } = await apiAuth.delete(
      `/marketing/lecture-events/${route.params.id}/participants/${item._id}`
    )
    if (data.success) {
      createSnackbar({
        text: '參加者刪除成功',
    ...
    createSnackbar({
      text: error?.response?.data?.message || '刪除參加者失敗',
      snackbarProps: { color: 'red-lighten-1' }
    })
  }
}
...
const openNoteHistoryDialog = async (item) => {
  currentHistoryParticipantId.value = item._id
...
const closeNoteHistoryDialog = () => {
  ...
  currentHistoryParticipantId.value = null
}
...
// 返回上一頁
const goBack = () => {
  router.push('/lectureEventManagement')
}

<template>
  <v-container max-width="2100">
    <v-row>
      <v-col
        ...
        class="pe-0"
      >
        <v-row class="elevation-4 rounded-lg pt-9 pb-9 px-1 px-sm-10 mt-2 mt-sm-6 ms-4 me-2 mx-xl-12 bg-white">
          <v-card
            width="100%"
            elevation="0"
          >
      ...
      <v-col xl="9">
        <v-row class="elevation-4 rounded-lg pt-3 pt-sm-5 pb-6 px-2 px-sm-4 mt-2 mt-sm-6 mx-0 mx-md-4 me-xl-12 mb-4 bg-white">
          <v-col
            cols="12"
          ...
          >
            ...
            >
              <v-card-title class="d-flex align-center mb-2">
                ...
                <span class="profile-sub-title">LINE 綁定狀態</span>
              </v-card-title>
  ...
  <ConfirmDialog
    ...
    dialog-width="320"
    title="解除 LINE 綁定"
    message="您確定要解除 LINE 帳號綁定嗎？解除後將無法繼續使用系統功能。"
...
const executeUnbind = async () => {
  try {
    ...
    if (data.success) {
      ...
      showUnbindConfirmDialog.value = false
      // 觸發 App.vue 重新檢查綁定狀態
...
onMounted(async () => {
  ...
  // 監聽 LINE 綁定狀態變化事件（用於綁定成功後即時更新）
  window.addEventListener('line-binding-status-changed', loadBindingStatus)
})

// 組件卸載時移除事件監聽
onUnmounted(() => {
  window.removeEventListener('line-binding-status-changed', loadBindingStatus)
})
</script>

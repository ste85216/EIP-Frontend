          ...
          <template v-else-if="batch">
            <div class="px-4 px-sm-6 pb-3 pt-3 d-flex align-center flex-wrap ga-2">
              <v-btn
                ...
                class="px-1"
                @click="router.push('/selfAssessmentStatistics')"
              >
                返回統計列表
              </v-btn>
            </div>
...
async function loadAll () {
  ...
  try {
    const [batchRes, overviewRes] = await Promise.all([
      ...
      apiAuth.get(`/evaluations/by-batch/${batchId.value}/self-assessment-overview-my-department`)
    ])
    ...
    router.push('/selfAssessmentStatistics')
  } finally {
    loading.value = false
  }
}
